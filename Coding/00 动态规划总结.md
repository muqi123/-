...
* [需要在做一遍的题](#需要在做一遍的题)
    ...
    * [2. 数组区间和](#1-数组区间和)
    * [3. 最长递增子序列](#1-最长递增子序列)
    * [4. 改变一组数的正负号使得它们的和为一给定数](#2-改变一组数的正负号使得它们的和为一给定数)
    * [5. 字符构成最多的字符串](#3-01-字符构成最多的字符串)
    * [6. 组合总和](#7-组合总和)
    * [7. 最长公共子序列](#1-最长公共子序列)
    * [8. 删除两个字符串的字符使它们相等](#1-删除两个字符串的字符使它们相等) 和7是一样的
<!-- GFM-TOC -->
...
这题也可以用segement tree来做，build O(n), update log(n), search log(n) + k, [huahua link](https://www.youtube.com/watch?v=rYBtViWXYeI/)
```c++
    struct SegTree
    {
        int start;
        ...
        SegTree* left;
        SegTree* right;
        SegTree( int s, int e, int sum): start(s),end(e), sum(sum), left(nullptr), right(nullptr){};
        SegTree( int s, int e, int sum, SegTree* left, SegTree* right): start(s),end(e), sum(sum), left(left), right(right){};
    };
    SegTree* buildTree( int start, int end, vector<int>& nums)
    {
        SegTree* root = nullptr;
        if( start == end ) return new SegTree( start, end, nums[start] );
        int mid = start + ( end - start ) / 2;
        SegTree* left = buildTree( start, mid, nums );
        SegTree* right = buildTree( mid + 1, end, nums );
        int sum = left->sum + right->sum;
        return new SegTree( start, end, sum, left, right );
    }
    ...
    int searchTree( SegTree* root, int i, int j )
    {
        int start = root->start, end = root->end; 
        if( start == i && end == j )
            return root->sum;
        int mid = start + ( end - start ) / 2;
        if( j <= mid )
            return searchTree( root->left, i, j );
        else if( i >= mid + 1 )
            return searchTree( root->right, i, j );
        else
            return searchTree( root->left, i, mid ) + searchTree( root->right, mid + 1, j );
    }
```
...
dp[i] 是sum等于i的最大乘积，计算结果要注意最大结果是either dp[i] or i.
``` c++
...
c++ 的解法，binary search. 用lower_bound，得到iterator。
得到iterator position iter - vec.gein() + 1
```c++
...
``` c++
    int wiggleMaxLength(vector<int>& nums) {
        ...
        return max( *max_element( pos.begin(), pos.end() ), 
                   *max_element( neg.begin(), neg.end() ) );
    }
```
        ...
        for( int i = 1; i <= m; i++ )
            for( int j = 1; j <= n; j++ )
            {
                if( text1[i-1] == text2[j-1] )
                    vec[i][j] = max( vec[i][j-1], vec[i-1][j-1] + 1 );
                else
